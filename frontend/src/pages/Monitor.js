...
function Monitor(props) {
  ...
  const openDialog = async (item) => {
    console.log(`${axios.get(`${Constants.AWS.STAGE}${Constants.AWS.APIs.SCREENSHOTPREVIEW}`);
      ...
      .queue([
        {
          ...
          html: (
            <img
              src={`data:image/png;base64, ${axios.get(`${Constants.AWS.STAGE}${Constants.AWS.APIs.SCREENSHOTPREVIEW}${item.url}`).then(console.log)}`}
            />
          ),
        },
        {
